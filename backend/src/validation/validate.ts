...
const messageFor = (id: string, label: string): string => {
  switch (id) {
...
export const validateSubmission = (payload: Partial<SubmissionInput>) => {
  const data = normalizeInput(payload as any);
  ...
  return { data: data as SubmissionInput, errors, valid: Object.keys(errors).length === 0 };
};
