...
export const normalizeInput = (payload) => {
    const out = { ...payload };
    if (out.aadhaarNumber)
        out.aadhaarNumber = out.aadhaarNumber.replace(/\s/g, "");
    if (out.panNumber)
        out.panNumber = out.panNumber.toUpperCase();
    if (out.ownerName)
        out.ownerName = out.ownerName.trim();
    if (out.otp)
        out.otp = out.otp.trim();
    return out;
};
